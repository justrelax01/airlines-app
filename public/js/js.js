...
function makeabooking() {
  ...
  var textfrom = document.getElementById("text-from").value;
  var textto = document.getElementById("text-to").value;
  ...
  tableRow.innerHTML = "\n      <td>".concat(fullname, "</td>\n      <td>").concat(email, "</td>\n      <td>").concat(age, "</td>\n      <td>").concat(phonenb, "</td>\n      <td>").concat(textfrom, "</td>\n      <td>").concat(textto, "</td>\n      <td>").concat(returndate, "</td>\n      <td>").concat(returntime, "</td>\n      <td>hotel 1</td>\n      <td><button onclick=\"deleteBooking(this)\">Delete</button></td>\n    ");

  // Append the table row to the table body
  var bookPostsContainer = document.getElementById("bookPostsContainer");
  ...
  var bookCountMessage = document.getElementById("bookCountMessage");
  bookCountMessage.style.display = "block";
  bookCountMessage.innerText = "Total Bookings: ".concat(bookPostsContainer.children.length);
}
...
function submitreg() {
  ...
  if (!pass.match(passRegex)) {
    errorMessages2.innerHTML = "Password should be between 8 and 30 characters,and must contain at least one lowercase letter,one uppercase letter,one digit,and one special character(!@#$%^&*()_+)";
    return;
  }
  if (pass !== passconfirm) {
